...
- Translator model not found ... Falling back to rule-based normalization.

Fix:

- Run python train_translator.py
- Ensure translator-model exists under ai_iss/translator-model

### 2) Legal model not found

Fix:

- Run python train.py
- Ensure legal-model exists under ai_iss/legal-model

### 3) Repetitive or broken text in answers

Current behavior:

- The generator should detect repetition and replace low-quality text automatically.

If it still appears:

- Re-test with a fresh app process
- Re-run stage-2 training on cleaner legal QA pairs

### 4) Arabic text looks garbled in terminal

On Windows PowerShell, set UTF-8 before running:
...
This project is for education and prototyping.
