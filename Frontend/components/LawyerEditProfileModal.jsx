...
export default function LawyerEditProfileModal({ 
  visible, 
  onClose, 
  C, 
  t, 
  ...
  const handleSave = () => {
    onSave({
      ...
      bio: bioRef.current,
      specialization: specRef.current,
      experience_years: expRef.current
    });
  };
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipScroll}>
              {SPECIALIZATION_OPTIONS.map(item => (
                <Pressable
                  ...
                  style={[styles.chip, currentSpec === item && { backgroundColor: C.accent, borderColor: C.accent }]}
                >
                  <Text style={[styles.chipText, { color: C.foreground }, currentSpec === item && { color: '#fff' }]}>{item}</Text>
                </Pressable>
              ))}
            </ScrollView>
